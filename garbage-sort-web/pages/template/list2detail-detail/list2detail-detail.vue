...
export default {
    ...
    onShareAppMessage() {
        return {
            title: this.banner.title,
            path:
                DETAIL_PAGE_PATH + '?detailDate=' + JSON.stringify(this.banner),
        };
    },
            ...
            uni.getProvider({
                ...
                success: (result) => {
                    ...
                    if (
                        ...
                        ~result.provider.indexOf('weixin')
                    ) {
    ...
    methods: {
        getDetail() {
            uni.request({
                url:
                    'https://unidemo.dcloud.net.cn/api/news/36kr/' +
                    this.banner.post_id,
                success: (data) => {
                    if (data.statusCode == 200) {
                        var htmlString = data.data.content
                            .replace(/\\/g, '')
