	...
	<view class="root">
		...
		>
			<view class="cards-swiper" v-if="!searchMode">
				<swiper :current="currentItem" :indicator-dots="false" :autoplay="false" next-margin="50rpx" previous-margin="50rpx" @change="onSwiperChange($event)">
					<swiper-item>
			...
			<scroll-view class="requirements" v-if="!searchMode" scroll-y="true">
		...
		<uni-popup ref="popup" type="bottom" background-color="#fff">
			<view class="selector-container">
				<view class="selector-header"><uni-title type="h1" title="选择城市"></uni-title></view>
				<view class="divider" />
...
export default {
	...
	methods: {
		...
		// 改变初始index为用户默认选中的城市
		getDefaultIndex() {
			const index = this.$data.citiesList.findIndex(e => e === this.$data.selectedCity);
			if (index > 0) {
				...
				axios
					...
					.then(res => {
						...
						if (res.data.data.length != 0) {
							// 更新本地存储
							this.commitSearchHistory(detail.value);
							// 显示数据
