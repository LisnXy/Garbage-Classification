        ...
        <view class="uni-padding-wrap">
            <view style="background: #fff; padding: 40rpx">
                ...
                <block v-if="hasUserInfo === true">
                    ...
                    <view
                        v-if="userInfo.avatarUrl || userInfo.avatar_url"
...
export default {
    ...
    computed: {
        ...mapState(['loginProvider', 'isUniverifyLogin']),
    },
    methods: {
        ...
        getUserInfo() {
            ...
            if (this.isUniverifyLogin) {
                // 一键登录
                this.getPhoneNumber(uni.getStorageSync('univerifyInfo'))
                    .then((phoneNumber) => {
                        ...
                        success: (res) => {
                            ...
                            if (!authStatus) {
                                uni.showModal({
                                    title: '授权失败',
                                    content:
                                        'Hello uni-app需要获取您的用户信息，请在设置界面打开相关权限',
                                    success: (res) => {
                                        if (res.confirm) {
                                            uni.openSetting();
                                        }
