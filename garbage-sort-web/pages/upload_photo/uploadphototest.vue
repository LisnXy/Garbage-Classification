<template>
	...
	<view class="root">
		<view class="header">
			...
			<image src="https://lisncloud-1311502437.cos.ap-shanghai.myqcloud.com/lanchao/image/upload-bg.png" mode="aspectFit" v-if="!ifChoosed" style="width: 100%;height:60%;position:relative;bottom: 20px;"></image>
		</view>

		<!-- 照相机图标部分 -->
		<view class="main-container" :style="{ height: containerHeight, position: containerPosition }">
			<view class="dropdown-btn-container" v-if="ifChoosed" @click="initPage"><image src="https://lisncloud-1311502437.cos.ap-shanghai.myqcloud.com/lanchao/icons/dropdown.svg" mode="aspectFit"></image></view>
			<view id="breath-btn" @click="ChooseImage()" v-if="!ifChoosed">
				<view id="inner" class="white-border"><image src="https://lisncloud-1311502437.cos.ap-shanghai.myqcloud.com/lanchao/image/照相机.svg" mode="aspectFit"></image></view>
			</view>
			<!-- 展示预选图部分 -->
			<view class="bg-img" v-for="(item, index) in imgList" :key="index" :data-url="imgList[index]">
				<image :src="imgList[index]" mode="aspectFit" style="width:100%;max-height: 100%;" @tap="ViewImage"></image>
				<view class="cu-tag bg-red" @tap.stop="DelImg" :data-index="index"><text class="cuIcon-close"></text></view>
				<uni-transition
					:duration="500"
					:mode-class="['slide-bottom', 'fade']"
					:show="showBtnContainer"
					style="width: 100%;display: flex;justify-content: center;"
					@change="resAniHandler"
				>
					<view class="reupload-btn-container">
						<view class="reupload-btn"><image src="https://lisncloud-1311502437.cos.ap-shanghai.myqcloud.com/lanchao/icons/刷新.svg" mode="aspectFit" @click="ChooseImage"></image></view>
					</view>
					<view class="button-container">
						<view class="single-btn" @click="UploadImage('single')">
							<text style="font-weight:500;">
								单目标
								<text style="color:gray;font-size:0.7rem;margin-left:5px;">快速识别</text>
							</text>
						</view>
						<view class="multi-btn" @click="UploadImage('multi')">
							<text style="font-weight:500;">
								多目标
								<text style="color:gray;font-size:0.7rem;margin-left:5px;">检测识别</text>
							</text>
						</view>
					</view>
				</uni-transition>
				<uni-transition :duration="500" mode-class="fade" :show="showResult" style="width:100%;">
					<scroll-view class="result-list-block" :scroll-y="true">
						<uni-list class="result-list">
							<uni-list-item v-for="(item, inde) in result" :key="inde" :title="item.label" :note="item.similarity" :clickable="false" :showArrow="false">
								<view slot="footer" class="list-item-icon" :style="{ color: bodyColors[item.type - 1] }">{{ mapName(item.type) }}</view>
							</uni-list-item>
						</uni-list>
					</scroll-view>
				</uni-transition>
		...
		<view v-if="ifChoosed" class="priview-title"><text>T</text></view>

		<tab-bar :selectedIndex="1" style="position:absolute;bottom: 0;width:100%;z-index: 2;" @switch-tab="initPage"></tab-bar>
	</view>
</template>
		...
		UploadImage(type) {
			...
			uni.uploadFile({
				...
				success: res => {
					...
					console.log(this.$store.state.user.isSaved);
					if (this.$store.state.user.isSaved) {
						axios.post('/user/updateSearchRecord', {
							openID: this.$store.state.user.openId,
							detectResults: this.result
						});
					}
