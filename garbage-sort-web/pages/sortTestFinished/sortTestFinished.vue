<template>
	<view class="finished-root">
		<view class="finished-header">
			...
			<image src="../../static/image/finished-bg.svg" mode="aspectFit"></image>
		</view>
		<view class="finished-body">
			<scroll-view class="list-container" scroll-y="true">
				<uni-list :border="false">
					<uni-list-item v-for="(item,index) in falseItems">
						<view class="list-item-header" slot="header" style="background-color: #F8EFEE;">
							<uni-icons type="closeempty" color="#B65454" size="25"></uni-icons>
						</view>
						<view class="list-item-body" slot="body">
							{{item.garbageName}}
						</view>
						<view class="list-item-footer" slot="footer" :style="{'border-color':bodyColors[item.type-1],'color':bodyColors[item.type-1]}">
							<text>{{getText(item.type)}}</text>
						</view>
					</uni-list-item>
					<uni-list-item v-for="(item,index) in correctItems">
						<view class="list-item-header" slot="header" style="background-color: #EDF7F4;">
							<uni-icons type="checkmarkempty" color="#86CBA5" size="25"></uni-icons>
						</view>
						<view class="list-item-body" slot="body">
							{{item.garbageName}}
						</view>
			...
			<view class="next-button-container">
				<button type="default" @click="nextBatch"><text>再测一次</text></button>
			</view>
		</view>
	</view>
</template>
