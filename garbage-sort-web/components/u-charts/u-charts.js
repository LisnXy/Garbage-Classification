...
function calLegendData(series, opts, config, chartData) {
    ...
    let lineHeight = Math.max(
        opts.legend.lineHeight * opts.pixelRatio,
        fontSize,
    );
    if (opts.legend.position == 'top' || opts.legend.position == 'bottom') {
        ...
        if (currentRow.length) {
            ...
            legendData.area.width =
                legendWidth - opts.legend.itemGap + 2 * padding;
...
function calCategoriesData(categories, opts, config, eachSpacing) {
    ...
    if (
        ...
        result.xAxisHeight =
            2 * config.xAxisTextPadding +
            maxTextLength * Math.sin(result.angle);
    }
    return result;
}
...
function getGaugeAxisPoints(categories, startAngle, endAngle) {
    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        categories[i]._endAngle_ =
            totalAngle * categories[i].value + startAngle;
...
function calTooltipYAxisData(point, series, opts, config, eachSpacing) {
    ...
    let item =
        maxVal - ((maxVal - minVal) * (point - minAxis)) / (maxAxis - minAxis);
    item = opts.yAxis.format ? opts.yAxis.format(Number(item)) : item;
    return item;
}
...
function drawRingTitle(opts, config, context, center) {
    ...
    if (subtitle) {
        ...
        var startY =
            center.y + subtitlefontSize / 2 + (opts.subtitle.offsetY || 0);
        if (title) {
    ...
    points.forEach(function (item, index) {
        if (item !== null) {
            ...
            context.fillText(
                String(formatVal),
                item.x -
                    measureText(formatVal, series.textSize || config.fontSize) /
                        2,
                item.y - 2,
            );
            context.closePath();
...
function drawPieText(series, opts, config, context, radius, center) {
    ...
    var seriesConvert = series.map(function (item) {
        ...
        var arc =
            2 * Math.PI -
            (item._start_ + (2 * Math.PI * item._proportion_) / 2);
...
function drawAreaDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        if (opts.dataPointShape !== false) {
            var shape =
                config.dataPointShape[
                    seriesIndex % config.dataPointShape.length
                ];
        ...
        if (config._xAxisTextAngle_ === 0) {
            newCategories.forEach(function (item, index) {
                ...
                context.fillText(
                    ...
                    startY +
                        xAxisFontSize +
                        (config.xAxisHeight - xAxisFontSize) / 2,
                );
                context.closePath();
                context.stroke();
            });
        ...
        if (opts.tooltip) {
            if (opts.tooltip.index == seriesIndex) {
                ...
                context.setFillStyle(
                    hexToRgb(
                        eachSeries.color,
                        opts.extra.pie.activeOpacity || 0.5,
                    ),
                );
...
function drawGaugeDataPoints(categories, series, opts, config, context) {
    ...
    for (
        let i = 0;
        i <
        gaugeOption.splitLine.splitNumber * gaugeOption.splitLine.childNumber +
            1;
        i++
    ) {
        context.beginPath();
...
function drawRadarDataPoints(series, opts, config, context) {
    ...
    var radius = Math.min(
        centerPosition.x -
            (getMaxTextListLength(opts.categories) +
...
function drawCharts(type, opts, config, context) {
    ...
    if (opts.categories && opts.categories.length) {
        opts.chartData.xAxisData = getXAxisPoints(
            opts.categories,
            opts,
            config,
        );
