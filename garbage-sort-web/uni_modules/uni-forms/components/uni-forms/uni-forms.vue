                ...
                let watch = this.$watch(
                    ...
                    (value) => {
                        ...
                        if (value.toString() === '[object Object]') {
                            for (let i in value) {
                                let name = `${key}[${i}]`;
                                this.formData[name] = this._getValue(
                                    name,
                                    value[i],
                                );
                            }
            ...
            if (this.validator) {
                for (let key in childrens) {
                    ...
                    if (child.isArray) {
                        ...
                        newFormData[name] = this._getValue(
                            name,
                            invalidFields[name],
                        );
                    }
                    const result = await child.triggerCheck(
                        invalidFields[name],
                        true,
                    );
                    if (result) {
                        results.push(result);
                        if (
                            this.errShowType === 'toast' ||
                            this.errShowType === 'modal'
                        )
                            break;
                    }
                }
