...
exports.main = async (event, context) => {
    ...
    if (event.headers) {
        ...
        platform = /iPhone|iPad/.test(event.headers)
            ? platform_iOS
            : platform_Android;
    } else if (context.OS) {
        platform =
    ...
    if (appid && appVersion && wgtVersion && platform) {
        ...
        if (record && record.data && record.data.length > 0) {
            const appVersionInDb =
                record.data.find((item) => item.type === package_app) || {};
            const wgtVersionInDb =
                record.data.find((item) => item.type === package_wgt) || {};
            ...
            let stablePublishDb = hasAppPackage
                ? hasWgtPackage
                    ? compare(wgtVersionInDb.version, appVersionInDb.version) >=
                      0
                        ? wgtVersionInDb
                        : appVersionInDb
                    : appVersionInDb
            ...
            if (Object.keys(stablePublishDb).length && appUpdate && wgtUpdate) {
                ...
                if (
                    min_uni_version &&
                    compare(min_uni_version, appVersion) < 1
                ) {
                    return {
                        code: 101,
                        message: 'wgt更新',
